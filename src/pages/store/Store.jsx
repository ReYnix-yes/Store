...
function Store() {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          image: g2,
          title:
            'Сифон АНИ для ванны с выпуском 1 1/2", с переливом, с гибкой трубой 40/50 E155',
          price: "580",
          id: "relatedProduct-2",
        },
        {
  ...
  function adminAdd() {
    if (currentType !== undefined && currentType !== null) {
      ...
      const selectedStore = updatedStoreData.find(
        (store) => store.type === currentType
      );
      setCurrentStore(selectedStore ? selectedStore.items : []);
    } else {
  ...
  const [registrationVisible, setRegistrationVisible] = useState("none");
  const [registrationLogin, setRegistrationLogin] = useState("");
  const [registrationPassword, setRegistrationPassword] = useState("");

  function newAccount() {
    const newLogin = registrationLogin.trim();
    const newPassword = registrationPassword.trim();
    ...
    if (userData.some((user) => user.access.login === newLogin)) {
  ...
  return (
    <>
      <div className="admin__new" style={{ display: adminAddVisible }}>
        <div className="admin__add__wrapper">
          <button
            onClick={() => setAdminAddVisible("none")}
          ...
          <div className="admin__add__content">
            ...
            <input
              ...
              name=""
              id=""
              className="photo__inp"
            />
          </div>
          <button className="admin__add__btn" onClick={adminAdd}>
            Добавить
          </button>
        </div>
      </div>
      <div className="registration" style={{ display: registrationVisible }}>
        <div className="registration__wrapper">
          <h1 className="registration__title">Новый логин:</h1>
          <input
            ...
            type="text"
          />
          <h1 className="registration__title">Новый пароль:</h1>
          <input
            ...
            type="text"
          />
          <button onClick={newAccount} className="create__btn">
            Создать аккаунт
          </button>
          <span
            onClick={() => setRegistrationVisible("none")}
      ...
      <div className="signin" style={{ display: signinVisible }}>
        <div className="signin__wrapper">
          <button
            onClick={() => setRegistrationVisible("flex")}
            className="registration__btn"
          >
            Регистрация
          </button>
          <span
            onClick={() => setSigninVisible("none")}
            className="signin__close"
          >
            Закрыть
          </span>
          ...
          />
          <button onClick={signin} className="signin__btn">
            Войти
          </button>
        </div>
      </div>
      ...
      <header>
        <nav className="nav">
          ...
          <ul className="nav__icon__wrapper">
            <h1>{user.name}</h1>
            <button
              onClick={() => setSigninVisible("flex")}
              className="nav__btn"
            >
              <img src={userIcon} alt="" className="nav__icon" />
            </button>

            <button onClick={openCart} className="nav__btn">
              <img src={cart} alt="" className="nav__icon" />
              <span
                className={`basket__amount ${
                  basketAmount > 0 ? "amount__active" : ""
                }`}
              >
                {basketAmount}
              </span>
        ...
        <nav className="catalog__links">
          <button
            onClick={() => handleStoreTypeClick("CastIronBathtubs")}
            className="catalog__link"
          >
            Чугунные ванны
          </button>
          <button
            onClick={() => handleStoreTypeClick("AcrylicBathtubs")}
            className="catalog__link"
          >
            Акриловаые ванны
          </button>
          <button
            onClick={() => handleStoreTypeClick("Sanitaryware")}
            className="catalog__link"
          >
            Санфаяс(раковины, унитазы)
          </button>
          <button
            onClick={() => handleStoreTypeClick("BathFurnitureStore")}
            className="catalog__link"
          >
            Мебель для ванны
          </button>
          <button
            onClick={() => handleStoreTypeClick("CastIronPallet")}
            className="catalog__link"
          >
            Чугунный поддон
          </button>
          <button
            onClick={() => handleStoreTypeClick("Polymer")}
            className="catalog__link"
          >
            Полимерпесчанные изделия
          </button>
          <button
            onClick={() => handleStoreTypeClick("RelatedProducts")}
            className="catalog__link"
          >
            Сопуствующие товары
          </button>
      ...
      <main className="store__main">
        <h1 className="store__type">
          {currentStore.length > 0 && currentStore[0].type}
        </h1>
        ...
        <div className="catalog">
          <div className="catalog__wrapper">
            ...
            <div style={{ display: adminEye }} id="admin">
              <button
                onClick={() => setAdminAddVisible("flex")}
